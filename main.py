        ...
        # [수정됨] AI 해설 요청 버튼 로직
            if st.button("🙋 Request Clarification", help="Ask AI why the other options are incorrect."):
                ...
                if 'model' not in globals() or model is None:
                    ...
                    # 2. 문제 문맥(Context) 데이터 준비
                    # 기본 지시문 (예: Fill in the blank...)
                    instruction_text = str(st.session_state.question_text)
                    
                    # [핵심 수정] 빈칸 문제라면 실제 예문(sentence)을 가져옴
                    if st.session_state.question_type == 'blank':
                        # 화면에 st.info()로 보여줬던 그 문장을 가져옵니다.
                        real_context = st.session_state.example_blank_to_show
                    else:
                        # 동의어 문제 등은 단어 자체가 문맥이 됨
                        real_context = f"Target Word: {word_text}"

                    # 3. Gemini 프롬프트 구성
                    prompt = f"""
                    I am an English learner studying TOEFL vocabulary.
                    I just answered a multiple-choice question and I need explanation.
                    
                    --- QUESTION DATA ---
                    1. **Type**: {st.session_state.question_type}
                    2. **Target Word (Answer)**: "{final_answer_text}"
                    3. **The Question Sentence (Context)**: "{real_context}"
                       (Note: This sentence contains the blank or context for the word.)
                    4. **Options Provided**: {st.session_state.quiz_options}
                    5. **My Selected Answer**: "{selected}"
                    ---------------------
                    
                    Please explain in Korean (but use English for specific terms):
                    1. **Context Analysis**: Look at the "Question Sentence". What clues in the sentence point to the correct answer?
                    2. **Why Correct**: Why is "{final_answer_text}" the best fit for this specific blank? (Explain definition and collocations).
                    3. **Why Incorrect**: Why is my choice "{selected}" (or other options) not appropriate in this sentence? Explain the nuance difference.
                    
                    Be concise and helpful.
                    """
                    
                    try:
                        with st.spinner("AI is analyzing the specific sentence context..."):
